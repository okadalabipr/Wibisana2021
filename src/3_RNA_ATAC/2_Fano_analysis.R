...
inner_join(fano_df, SE, by = "ensembl_gene_id") %>%
  ...
  geom_hline(yintercept = 0)+
  guides(color = F)+
  geom_text_repel(inner_join(fano_df, SE, by = "ensembl_gene_id") %>% filter(label !=0), min.segment.length = 0, mapping = aes(x=l2fc, y=log2(fano_10/fano_0), label = external_gene_name.x), size = 8.0, nudge_y = 3, nudge_x = 0, segment.alpha = 0.7, color = "black")+
  cowplot::theme_cowplot(font_size = 40, line_size = 1)


ggsave("SE_allgenes.svg", width = 7, height = 7)
...
ggsave("TE_allgenes.svg", width = 7, height = 7)



#  fano and expression corr 
fano_df %>% ggplot(aes(x = log2(mean_10/mean_0), y = log2((fano_10*mean_10)/(fano_0*mean_0)))) + 
  geom_point(size = 3, alpha = 0.5)  + 
  stat_smooth(method = "lm", col = "black") +
  stat_cor(method = "spearman", size = 10, label.y = 30) +
  cowplot::theme_cowplot(font_size = 40, line_size = 1)

ggsave("mean_fano.svg", width = 7, height = 7)
